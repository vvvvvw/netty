 ...
 */
//直接使用字节数组,基于堆内存进行内存分配的字节缓冲区，没有基于对象池创建
    //特点:
    //1.动态扩展缓冲区
public class UnpooledHeapByteBuf extends AbstractReferenceCountedByteBuf {
    ...
    public ByteBuf capacity(int newCapacity) {
        ...
        if (newCapacity > oldCapacity) {
            ...
            //新的容量小于当前的缓冲区容量，创建一个新的子缓冲区
            byte[] newArray = allocateArray(newCapacity);
            int readerIndex = readerIndex();
            if (readerIndex < newCapacity) {//判断读索引是否小于新的容量值
                int writerIndex = writerIndex();
                if (writerIndex > newCapacity) {//如果小于进一步判断写索引是否大于新的容量值
                    writerIndex(writerIndex = newCapacity);//如果大于则将写索引设置为新的容量值
                }
                System.arraycopy(oldArray, readerIndex, newArray, readerIndex, writerIndex - readerIndex);//复制
            } else {
                //读索引大于等于新的容量值，则只设置 读写索引为新容量值
