...
public class PooledByteBufAllocator extends AbstractByteBufAllocator implements ByteBufAllocatorMetricProvider {
    ...
    static {
        ...
        //nHeapArena参数来执行Arena的数量，或者通过key为io.netty.allocator.numHeapArenas的系统变量进行设置，它有一个默认值
    ...
    protected ByteBuf newDirectBuffer(int initialCapacity, int maxCapacity) {
        ...
        if (directArena != null) {
            ...
            // 如果当前缓存中由于某种原因无法获取到directArena，则直接创建一个存有直接内存的非池化的ByteBuf，
            // 一般情况下不会走到这一步
            buf = PlatformDependent.hasUnsafe() ?
    ...
    final class PoolThreadLocalCache extends FastThreadLocal<PoolThreadCache> {
        ...
        protected synchronized PoolThreadCache initialValue() {
            // 获取对应的PoolArena数组中最少被使用的那个Arena，将其返回。
            ...
            //如果不启用 线程本地缓存功能，则直接初始化一个 所有值都为0的 线程本地缓存
        ...
        /*
         获取对应的PoolArena数组中被最少的线程当做本地缓存的Arena
         实现: 通过比较PoolArena.numThreadCaches属性来进行的，该属性记录了当前PoolArena被
