...
public class ChannelTrafficShapingHandler extends AbstractTrafficShapingHandler {
    ...
    //handlerremove的时候，
    // 1.如果channel 连接状态是好的，则将messagesQueue中未写的 数据写入到channel中
    // 2.如果channel 不在连接，则释放 所有 未写的 数据 占据的空间
    // 开启读操作，开启写操作
    ...
    private static final class ToSend {
        //可写入到 ChannelOutboundBuffer 的时间
    ...
    void submitWrite(final ChannelHandlerContext ctx, final Object msg,
        ...
        // write order control
        synchronized (this) {
            //如果 延迟时间为0(立即发生)，并且messagesQueue队列为空,则立即发送
