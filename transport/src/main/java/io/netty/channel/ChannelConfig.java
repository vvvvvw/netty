...
public interface ChannelConfig {
     ...
     */
    /*
    返回写入操作的最大循环计数，直到{@link WritableByteChannel＃write（ByteBuffer）}返回一个非零值。 它类似于在并发编程
    中使用自旋锁。 它可以提高内存利用率并根据运行JVM的平台来编写吞吐量。 默认值是{code 16}。
     */
     ...
     */
    //当且仅当 ChannelHandlerContext.read() 自动调用而无需 用户调用时 返回true
    boolean isAutoRead();
